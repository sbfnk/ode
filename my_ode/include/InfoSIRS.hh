...
struct InfoSIRSParams
{
  InfoSIRSParams() : nvars(2) {};
  ...
  double tau[2][2], alpha, nu, lambda;
  double omega;
  double sigma;
  ...
  friend std::ostream& operator <<
    (std::ostream& os, const InfoSIRSParams& x);
      
  // overloading operator=
  void operator= (const InfoSIRSParams& x);
      
}; // InfoSIRSParams

//------------------------------------------------------------

void InfoSIRSParams::init_Q(double* rhs, bool verbose)
{   
  ...
  if (sigma > 0) {
    tau[1][0] = sigma*tau[0][0];
    tau[1][1] = sigma*tau[0][1];
    if (verbose) {
      std::cout << "sigma given, setting tau+-=" << tau[1][0] << " and "
                << "tau++= " << tau[1][1] << std::endl;
    }
  }
...
void InfoSIRSParams::operator= (const InfoSIRSParams& x)
{
  int i,j,k;
...
std::ostream& operator<< (std::ostream& os, const InfoSIRSParams& x)
{
  os << std::endl
     ...
     << "qid    = " << x.qid << std::endl
     << "-----------------" << std::endl
     << "R0d -- = " << (x.tau[0][0])/(x.gamma[0]) << std::endl
     << "R0d -+ = " << (x.tau[0][1])/(x.gamma[0]) << std::endl
     << "R0d +- = " << (x.tau[1][0])/(x.gamma[1]) << std::endl
     << "R0d ++ = " << (x.tau[1][1])/(x.gamma[1]) << std::endl
     << "R0i    = " << (x.alpha)/(x.lambda) << std::endl
     << "-----------------" << std::endl
...
struct InfoSIRSMeanField
{
  // rhs function
  static int rhs_eval (double t, const double y[], double rhs[], void* params)
  {         
    InfoSIRSParams p = *(static_cast<InfoSIRSParams*>(params));
    
    // local readable short variables
    double bd=p.tau[0][0], gd=p.gamma[0], dd=p.delta[0];
    double bi=p.tau[1][1], gi=p.gamma[1], di=p.delta[1];
    double bm1=p.tau[0][1], bm2=p.tau[1][0], al=p.alpha, nu=p.nu;
    double lm=p.lambda, om=p.omega;
    double N=p.N, invN=1./N;
    //double Qd=p.Qd, Qi=p.Qi;

    // scaling is done in main
...
struct InfoSIRSdiPairApprox
{
  ...
  static int rhs_eval (double t, const double y[], double rhs[], void* params)
  {
    ...
    double bd=p.tau[0][0], gd=p.gamma[0], dd=p.delta[0];
    double bi=p.tau[1][1], gi=p.gamma[1], di=p.delta[1];
    double b1=p.tau[0][1], b2=p.tau[1][0], al=p.alpha, nu=p.nu;
    double lm=p.lambda, om=p.omega;
...
}; // InfoSIRSdiPairApprox

//------------------------------------------------------------

struct InfoSIRSdibPairApprox
{
  // pair approx      
  static double pa(double kk, double ij, double jk, double j)
  {
