...
namespace InfoSIRSpa
{
  ...
  struct Params
  {
    ...
    // clustering
    double Cddd, Cddi, Cdid, Cdii, Ciid, Ciii;
    double       Cidi, Cidd;
  ...
  std::ostream& operator<< (std::ostream& os, const Params& x)
  {
    os << std::endl
       ...
       << "qid    = " << x.qid << std::endl
       << "-----------------" << std::endl
       << "Cddd  = " << x.Cddd << std::endl
       << "Cddi  = " << x.Cddi << std::endl
       << "Cdid  = " << x.Cdid << std::endl
       << "Cdii  = " << x.Cdii << std::endl
       << "Ciid  = " << x.Ciid << std::endl
       << "Ciii  = " << x.Ciii << std::endl
       << "Cidi  = " << x.Cidi << std::endl
       << "Cidd  = " << x.Cidd << std::endl
       << "-----------------" << std::endl;
  ...
  po::options_description* generate_model_options(ode::OdeSolver<Params, Eqs>& dummy)
  {
    ...
    opt->add_options()
      ...
      //------------------------------------------------
//       ("eps", po::value<double>(),
//        "threshold value for evaluating clustering coefficient correction")
      ;
                        ...
                        ode::OdeSolver<Params, Eqs>& x)
  {
    ...
    //------------------------------------------------        
    if (vm.count("Cddd")) {
      model_params->Cddd=vm["Cddd"].as<double>();
    } else {
      std::cerr << "WARNING: no Cddd given" << std::endl;
      std::cerr << "setting to 0" << std::endl;
      model_params->Cddd=0;
    }
    if (vm.count("Cddi")) {
      model_params->Cddi=vm["Cddi"].as<double>();
    } else {
      std::cerr << "WARNING: no Cddi given" << std::endl;
      std::cerr << "setting to 0" << std::endl;
      model_params->Cddi=0;
    }
    if (vm.count("Cdid")) {
      model_params->Cdid=vm["Cdid"].as<double>();
    } else {
      std::cerr << "WARNING: no Cdid given" << std::endl;
      std::cerr << "setting to 0" << std::endl;
      model_params->Cdid=0;
    }
    if (vm.count("Cdii")) {
      model_params->Cdii=vm["Cdii"].as<double>();
    } else {
      std::cerr << "WARNING: no Cdii given" << std::endl;
      std::cerr << "setting to 0" << std::endl;
      model_params->Cdii=0;
    }
    if (vm.count("Ciid")) {
      model_params->Ciid=vm["Ciid"].as<double>();
    } else {
      std::cerr << "WARNING: no Ciid given" << std::endl;
      std::cerr << "setting to 0" << std::endl;
      model_params->Ciid=0;
    }
    if (vm.count("Ciii")) {
      model_params->Ciii=vm["Ciii"].as<double>();
    } else {
      std::cerr << "WARNING: no Ciii given" << std::endl;
      std::cerr << "setting to 0" << std::endl;
      model_params->Ciii=0;
    }
    model_params->Cidi = model_params->Cdii;
    model_params->Cidd = model_params->Cdid;
